...
fn press_button(count: usize, modules: &mut HashMap<String, Module>) -> HashMap<Signal, usize> {
    ...
    for i in 1..=count {
        ...
        while let Some(pulse) = pulse_queue.pop_front() {
            if matches!(pulse.from.as_str(), "br" | "rz" | "fk" | "lf")
                && pulse.signal == Signal::High
            {
                println!("button count for {} sending high: {}", pulse.from, i);
            }
...
fn mermaid_chart(modules: &mut HashMap<String, Module>) -> String {
    let mut output = String::from("graph TD;\n");

    for module in modules.values() {
        for output_module in module.outputs.clone() {
            output.write_fmt(format_args!("   {}-->{}\n", module.id, output_module));
        }
    }
...
fn process(input: &str) -> String {
    ...
    println!("{}", mermaid_chart(&mut modules));

    // dbg!(&modules);

    let results = press_button(10000000, &mut modules);
